...
import { StarSystem, Probe, SimulationConfig, SettlementStatus, Vector3D } from './types';
import { periodicDistance, subtractVectors, scaleVector } from './utils';
...
export function calculateInterceptTime(
  ...
  const relPos: Vector3D = { x: pdx, y: pdy, z: pdz };

  // Relative velocity (target velocity - source velocity)
  const relVel = subtractVectors(target.velocity, source.velocity);
  ...
  const relVelLyPerYear = scaleVector(relVel, (1 / 299792.458));

  // Quadratic equation: a*t^2 + b*t + c = 0
  // where we're solving |relPos + relVel*t|^2 = (v_probe*t)^2
  const a =
    relVelLyPerYear.x * relVelLyPerYear.x +
    relVelLyPerYear.y * relVelLyPerYear.y +
    relVelLyPerYear.z * relVelLyPerYear.z -
    probeVelLyPerYear * probeVelLyPerYear;

  const b =
    2 * (relPos.x * relVelLyPerYear.x + relPos.y * relVelLyPerYear.y + relPos.z * relVelLyPerYear.z);
...
export function findBestTarget(
  ...
  for (const target of systems) {
    ...
    if (!target.settleable) continue;

    // Skip if already settled or targeted
    if (target.status !== SettlementStatus.UNSETTLED) continue;
    ...
    if (interceptTime === null) continue;
